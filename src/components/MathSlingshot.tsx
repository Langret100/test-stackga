 ...
 * ✦ 데스크탑: 웹캠 손 추적 (MediaPipe)
 * ✦ 모바일:   터치 드래그로 슬링샷 조작
 */

import React, { useEffect, useRef, useState, useCallback } from 'react';
import { Point, Bubble, Particle, BubbleColor, GameMode } from '../types';
import { Loader2, Trophy, Play, MousePointerClick, Zap, RefreshCw, X, Users } from 'lucide-react';
import {
...
const COLOR_CONFIG: Record<BubbleColor, { hex:string; dark:string; points:number }> = {
  red:    { hex:'#ff6b6b', dark:'#c0392b', points:100  },
  blue:   { hex:'#4fc3f7', dark:'#0277bd', points:150  },
  green:  { hex:'#81c784', dark:'#2e7d32', points:200  },
  yellow: { hex:'#fff176', dark:'#f57f17', points:250  },
  purple: { hex:'#ce93d8', dark:'#6a1b9a', points:300  },
  orange: { hex:'#ffb74d', dark:'#e65100', points:350  },
};
...
const drawMarble = (
  ...
  ctx.shadowColor = `${dark}99`; ctx.shadowBlur = 6;
...
const MathSlingshot: React.FC = () => {
  ...
  const initGrid = useCallback((width: number) => {
    ...
    bubbles.current=[...nb]; particles.current=[]; dustParticles.current=[];
  ...
  const createExplosion = (x: number, y: number, color: string) => {
    ...
    for (let i=0;i<12;i++) particles.current.push({x,y,vx:(Math.random()-.5)*14,vy:(Math.random()-.5)*14,life:1.0,color});
    for (let i=0;i<5;i++)  particles.current.push({x,y,vx:(Math.random()-.5)*8,vy:-Math.random()*10-2,life:1.2,color:'#fff'});
  };
  ...
  const checkMatches = (start: Bubble) => {
    ...
    while (stack.length>0) {
      const cur=stack.pop()!; if (visited.has(cur.id)) continue; visited.add(cur.id);
    ...
    if (matches.length>=3) {
      ...
      scoreRef.current+=Math.floor(total*mult); setScore(scoreRef.current);
      return true;
    }
    return false;
  };
  ...
  const addNewRow = (cw: number) => {
    ...
    bubbles.current.push(...makeRow(0,cw));
  ...
  // ── Launch logic (공통) ───────────────────────────────────────────────────
  const tryLaunch = (now: number, isHard: boolean) => {
  ...
  // ── Slingshot drag (공통 draw-frame에서 사용) ─────────────────────────────
  ...
  const renderFrame = useCallback((
    ...
    // ── Hand (desktop) ──
    if (!isMob && handPos) {
      if (frameCount%2===0&&window.drawConnectors&&window.drawLandmarks) {
        // landmarks는 onResults에서만 그리므로 여기선 커서만
      }
    ...
    // ── Mobile touch input ──
    if (isMob) {
      if (!isLocked&&touchActiveRef.current&&!isFlying.current) {
        isPinching.current=true; applyDrag(touchPosRef.current);
      } else if (isPinching.current&&(!touchActiveRef.current||isLocked)) {
        isPinching.current=false;
        if (isLocked) ballPos.current={...anchorPos.current};
        else tryLaunch(now, isHard);
      } else if (!isFlying.current&&!isPinching.current) {
    ...
    if (isFlying.current) {
      if (now-flightStart.current>5000) {
        ...
        if (hit) {
          ...
          bubbles.current.push(nb); checkMatches(nb); updateAvailableAnswers();
    ...
    for (const b of bubbles.current) {
      if (b.active) drawMarble(ctx,b.x,b.y,BUBBLE_RADIUS-1,b.color,b.expression,isHard,1.0,doShake?gsx:0,doShake?gsy:0);
    }
    ...
    // ── 모바일: 터치 조준선 ──
    if (isMob&&isPinching.current) {
      ctx.save();
      ctx.setLineDash([12,10]);
      ctx.lineDashOffset=-((now/12)%22);
      ctx.strokeStyle='rgba(255,255,255,0.35)'; ctx.lineWidth=2;
      // 반대 방향으로 예측선
      const pdx=anchorPos.current.x-ballPos.current.x, pdy=anchorPos.current.y-ballPos.current.y;
      const plen=Math.sqrt(pdx*pdx+pdy*pdy);
      if (plen>10) {
        ctx.beginPath();
        ctx.moveTo(ballPos.current.x,ballPos.current.y);
        ctx.lineTo(ballPos.current.x+pdx/plen*150, ballPos.current.y+pdy/plen*150);
        ctx.stroke();
      }
      ctx.restore();
    }
    ...
    if (oppBubblesRef.current.length>0&&multiStatusRef.current==='playing') {
      const ow=isMob?120:160, oh=isMob?160:220;
  ...
  const startMultiplayer = useCallback(async () => {
    ...
    try {
      ...
      oppUnsubRef.current=subscribeOppState({...fb,roomId:joined.roomId,pid:joined.pid,onOpp:(res)=>{
        ...
        oppBubblesRef.current=res.state.bubbles; setShowMatchedBanner(false);
      }});
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // MAIN EFFECT: 데스크탑(MediaPipe) / 모바일(rAF) 분기
  // ─────────────────────────────────────────────────────────────────────────
  useEffect(()=>{
    if (!canvasRef.current||!gameContainerRef.current) return;
    const canvas=canvasRef.current, container=gameContainerRef.current;
    ...
    // ── 모바일: rAF 루프 (웹캠 없이 solid 배경) ──────────────────────────
    if (mobile) {
      setLoading(false);
      let handPosM: Point|null=null;
      const loop=()=>{
        ...
        renderFrame(ctx,canvas,null,handPosM,1.0,performance.now(),frameCountRef.current);
    ...
    const onResults=(results: any)=>{
      ...
      renderFrame(ctx,canvas,results.image,handPos,pinchDist,performance.now(),frameCountRef.current);
    };
  ...
  useEffect(()=>{
    ...
    const onTouchStart=(e: TouchEvent)=>{
      ...
      if (Math.sqrt(dx*dx+dy*dy)<150) {
        touchActiveRef.current=true; touchPosRef.current=pos;
      }
    };
    const onTouchMove=(e: TouchEvent)=>{ e.preventDefault(); if(touchActiveRef.current) touchPosRef.current=getTouchPos(e); };
    const onTouchEnd=(e: TouchEvent)=>{ e.preventDefault(); touchActiveRef.current=false; };
    ...
    <div className="w-full h-screen bg-[#121212] overflow-hidden text-[#e3e3e3] relative select-none"
        ...
        <div className="absolute inset-0 z-40 flex items-center justify-center"
          ...
          <div className="flex flex-col items-center gap-6 px-4 w-full max-w-sm">
            <div className="text-center">
              <div className="text-5xl mb-2 select-none" style={{filter:'drop-shadow(0 0 20px #4fc3f7)'}}>🧮</div>
              <h1 className="text-4xl font-black mb-1" style={{
                background:'linear-gradient(135deg,#4fc3f7,#81c784,#ffb74d)',
                WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',
              }}>매스 슬링샷</h1>
              <p className="text-[#c4c7c5] text-sm">같은 정답의 구슬을 3개 이상 연결하세요!</p>
            </div>
            <div className="bg-white/5 rounded-2xl p-4 border border-white/10 text-xs text-[#c4c7c5] w-full space-y-1.5">
              <p>🎯 같은 정답 구슬 3개 이상 → 팡!</p>
              <p>📉 아래로 3번 쏘면 하드모드 (점수 1.5배)</p>
              <p>⏱️ 12초마다 새 줄이 내려옵니다</p>
              <p>💥 구슬이 발사대에 닿으면 Game Over</p>
              {isMobile&&<p className="text-[#4fc3f7]">📱 슬링샷 근처를 터치해서 당겨 발사!</p>}
              {!isMobile&&<p className="text-[#4fc3f7]">🖐️ 손가락을 모아서 구슬을 당겨 발사!</p>}
            </div>
            <button onClick={handleStart}
              className="w-full py-4 rounded-2xl font-black text-2xl transition-all duration-200 active:scale-95 select-none"
              style={{
      ...
      {showHardNotif&&(
        <div className="absolute inset-0 flex items-center justify-center z-50 pointer-events-none">
          <div className="bg-gradient-to-br from-[#ab47bc] to-[#ef5350] p-6 rounded-3xl shadow-2xl text-center animate-bounce">
            ...
            <p className="text-white/90 font-bold">점수 1.5배!</p>
          </div>
        </div>
      )}
      ...
      {gamePhase==='playing'&&(
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-40">
          <div className="bg-[#1e1e1e]/90 px-4 py-3 rounded-[24px] border border-[#444746] shadow-2xl flex items-center gap-2 backdrop-blur-sm">
            <p className="text-[9px] text-[#c4c7c5] uppercase font-bold tracking-wider mr-1 hidden sm:block whitespace-nowrap">발사할 정답</p>
      ...
      {/* 모바일 조작 힌트 */}
      {isMobile&&gamePhase==='playing'&&!isGameOver&&!isPinching.current&&!isFlying.current&&(
        <div className="absolute bottom-20 left-1/2 -translate-x-1/2 z-30 pointer-events-none opacity-40">
          <div className="flex items-center gap-2 bg-[#1e1e1e]/90 px-3 py-1.5 rounded-full border border-[#444746]">
            <Play className="w-3 h-3 text-[#42a5f5] fill-current"/>
            <p className="text-xs font-medium">슬링샷 근처를 터치해서 당기세요</p>
          </div>
        </div>
      )}
